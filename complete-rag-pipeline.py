...
def main():
    ...
    text_loader = TextLoader("/Users/harrisishfaq/Desktop/PycharmProjects/g_ai/mogli-story.txt")
    documents = text_loader.load()


    print("INGESTION 2nd step: Splitting...")
    splitting = CharacterTextSplitter(chunk_size=1000, chunk_overlap=50)
    texts = splitting.split_documents(documents)
    ...
    if False: # Change to True if you want to re-ingest documents
        vector_store = PineconeVectorStore.from_documents(texts, embeddings, index_name=os.environ.get("MOGLI_INDEX_NAME"))
    else:
        vector_store = PineconeVectorStore(embedding=embeddings, index_name=os.environ.get("MOGLI_INDEX_NAME"))
